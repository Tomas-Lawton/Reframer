var OffsetUtils = {
    offsetPath: function(path, offset, result) {
        ...
        if (path.isClosed()) {
            if (!outerPath.lastSegment.point.isClose(
    ...
    offsetCurve_middle: function(curve, offset) {
        var v = curve.getValues(),
            ...
            pt = Curve.getPoint(v, 0.5).add(Curve.getNormal(v, 0.5).multiply(offset)),
    ...
    offsetCurve_average: function(curve, offset) {
        var v = curve.getValues(),
            ...
            u = 1 - t,
            pt = Curve.getPoint(v, t).add(Curve.getNormal(v, t).multiply(offset)),
            t1 = Curve.getTangent(v, 0),
            t2 = Curve.getTangent(v, 1),
